...
# Some part of this code is taken from my previouse Obligs.
...
parser.add_argument('-p', '--port', type=InputValidation.valid_port, default=8088, help='Sets the port adress the server will listen to, must be integer and in the range [1024,65535]')
parser.add_argument('-i', '--ip', default='127.0.0.1', type=InputValidation.valid_ip, help='Sets the the ip adress the server in run from/the client want to connect to')
parser.add_argument('-f', '--filename', type=InputValidation.valid_filename, default='image.jpg', help='The filepath to the file you want to tranfere')
parser.add_argument('-w', '--windowsize', type=InputValidation.valid_window, default = 3 , help='Sets the sliding window size')
parser.add_argument('-d', '--discard', type=InputValidation.valid_discard_packet, default=-1, help='Sets the sequence number for the packet to be discarded on the server side')
